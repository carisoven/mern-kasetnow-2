...
router.post("/addproduct", auth, upload.single('image'), async (req, res) => {
    try {
        // * Refer on input data
        const { shopid, productname, producttype, price, invent } = req.body;
        ...
        // ! Add Product in Shop
        const shopupdate = await Shop.updateOne({ shopname: shop2.shopname },
            {
                $push: {
                    product: {
                        _id: products.id,
...
router.post("/edit/:productid", auth, upload.single('image'), async (req, res) => {
    try {
        const productedit = await Product.findById(req.params.productid).populate('shop');
        const image = req.file;
        ...
        console.log(product);
        const productupdate = await Product.findOneAndUpdate({ _id: productedit.id }, { $set: product }, { new: true });
...
router.post("/addstock/:productid", auth, async (req, res) => {
    try {
        const searchproduct = await Product.findById(req.params.productid).populate('shop');
        const { amount } = req.body;
        console.log(searchproduct);
        const stock = {
            amount: searchproduct.amount + amount
        }
        const addstock = await Product.findByIdAndUpdate(searchproduct.id, { $set: stock }, { new: true });
...
router.post("/sa-addproduct/:id", auth, upload.single('image'), async (req, res) => {
    try {
        // * Refer on input data
        const { productname, producttype, price, invent } = req.body;
